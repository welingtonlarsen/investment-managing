...
const RegistrationForm = () => {
  ...
  const { handleSubmit: ordersHandleSubmit } = useForm<OrdersInput>({
    resolver: yupResolver(schemaOrders),
  })
  const { handleSubmit: businessSumaryHandleSubmit } = useForm<any>({
    resolver: yupResolver(schemaOrders),
  })
      ...
      <Container component="main" maxWidth="md" sx={{ mb: 4 }}>
        <Paper variant="outlined" sx={{ my: { xs: 3, md: 6 }, p: { xs: 2, md: 3 } }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
            ...
            <Button variant="contained" onClick={() => handleNextSubmit(activeStep)} sx={{ mt: 3, ml: 1 }}>
