    ...
    return [
        {
            ...
            useFactory: async (dependenciesProvider: BrokerageOrderRepoDependenciesProvider) =>
...
async function provideBrokerageOrderRepositoryFactory(dependenciesProvider: BrokerageOrderRepoDependenciesProvider) {
    await ConfigModule.envVariablesLoaded;
    switch(process.env.DATABASE_DATASOURCE) {
        case DataSource.TYPEORM: 
            return new BrokerageOrderTypeormRepository(dependenciesProvider.typeOrmRepository)
        default:
...
export class BrokerageOrderRepoDependenciesProvider {
    constructor(@InjectRepository(BrokerageOrder)
