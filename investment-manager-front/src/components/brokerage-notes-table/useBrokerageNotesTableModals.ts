...
type ModalsAction = { type: 'OPEN_DELETE_CONFIRMATION_MODAL', selectedItem: number } | { type: 'CLOSE_DELETE_CONFIRMATION_MODAL' } |
  { type: 'OPEN_BROKERAGE_NOTE_MODAL', selectedItem: number } | { type: 'CLOSE_BROKERAGE_NOTE_MODAL' } | { type: 'CLOSE_ALL_MODALS' }
...
const reducer = (state: ModalsStates, action: ModalsAction): ModalsStates => {
  switch (action.type) {
    ...
    case "CLOSE_ALL_MODALS": {
      return { ...state, isBrokerageNoteModalOpened: false, isDeleteConfirmationModalOpened: false, selectedItem: null }
    }
    default:
...
export const useBrokerageNotesTableModals = () => {
  const [modalsStates, dispatch] =
    useReducer(reducer, {isDeleteConfirmationModalOpened: false, isBrokerageNoteModalOpened: false, selectedItem: null})
